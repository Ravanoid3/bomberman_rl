"""Run matches as subprocesses and collect their statistics.
That's it, that's what it does.

Matches are independent, so they are dispatched across processes; the framework
itself is single-threaded and single-round-at-a-time.
"""
...
class Match:
    ...
    #: Extra environment for this match -- how a variant is selected, see
    #: ``agent_code/q_agent/api.py:resolve_config``.
    ...
    def command(self):
        ...
        cmd = [sys.executable, str(REPO / 'main.py'), 'play', '--no-gui',
...
TRANSIENT_MARKERS = ('WinError 32', 'WinError 5', 'PermissionError')


def is_transient(output):
...
def run(match, timeout=None, verbose=False, retries=0, on_retry=None):
    ...
    while True:
        ...
        transient = proc.returncode != 0 and is_transient(output)
        if attempt < retries and transient:
            attempt += 1
            print(f'  .. match {match.label!r} hit a transient filesystem '
                  f'error, retrying ({attempt}/{retries})', flush=True)
            if on_retry is not None:
                replacement = on_retry(attempt)
...
# metric extraction
# --------------------------------------------------------------------------
def agent_metrics(stats, agent_name):
...
def round_records_world_totals(stats):
    """Per-round rows (``steps``, ``coins``, ``kills``, ``suicides``).
    These are world totals, so they are only attributable to a single agent in
    solo scenarios.
    """
    rows = []
