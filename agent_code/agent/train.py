...
PROGRESS_STALLED  = "PROGRESS_STALLED"  # no obj-dist improvement for many steps

GAME_REWARDS = {
    # coins
    e.COIN_COLLECTED:       30,
    CLOSER_TO_COIN:          2.0,
    FARTHER_FROM_COIN:      -2.0,

    # crates
    e.CRATE_DESTROYED:       4,
    e.COIN_FOUND:            5,
    CLOSER_TO_CRATE:         0.8,
    FARTHER_FROM_CRATE:     -0.8,

    # -- opponents --
    e.KILLED_OPPONENT:      80,
    e.OPPONENT_ELIMINATED:   0,
    CLOSER_TO_OPP:           2.5,
    FARTHER_FROM_OPP:       -1.0,
    BOMB_ON_OPP:             8.0,
    BOMB_ON_CRATE:           2.0,
...
def setup_training(self):
    ...
    # per-round game-stat counters (reset each round in end_of_round)
    self.round_coins     = 0
    self.round_kills     = 0
...
def derive_shaping_events(self, old_state, new_state, action):
    ...
    if not was_in_danger:
        ...
        if cleanup_mode:
            ...
            _, od_opp = bfs_dir_dist(free_hunt, old_pos, old_opps_xy)
            _, nd_opp = bfs_dir_dist(free_new_hunt, new_pos, old_opps_xy)
            if od_opp is not None and nd_opp is not None:
                if nd_opp < od_opp:
                    custom.append(CLOSER_TO_OPP)
                elif nd_opp > od_opp:
                    custom.append(FARTHER_FROM_OPP)
    ...
    if objectives_remain and not was_in_danger and can_move(old_state):
        ...
        swp = getattr(self, 'steps_without_progress', 0)
        if swp >= 5:
            custom.append(PROGRESS_STALLED)
    self.pos_history.append(new_pos)
...
def store_and_learn(self, old_state, action, new_state, events, done):
    ...
    if feat_old is not None:
        for feat_s, feat_ns, act_s in augment(feat_old, feat_new, action_idx):
            self.replay_buffer.append(Transition(feat_s, act_s, feat_ns, reward, done))
...
def build_dir_groups():
    from .callbacks import FEATURE_NAMES, DIR_ORDER
    groups = []
    i = 0
    while i < len(FEATURE_NAMES) - 3:
        names = FEATURE_NAMES[i:i+4]
...
def learn_from_batch(self):
    ...
    q_current  = np.sum((states @ self.model.T) * np.eye(len(ACTIONS))[actions], axis=1)
...
def game_events_occurred(self, old_game_state, self_action, new_game_state, events: List[str]):
    ...
    events = list(events) + derive_shaping_events(self, old_game_state, new_game_state, self_action)
...
def end_of_round(self, last_game_state, last_action, events: List[str]):
    ...
    events = list(events) + derive_shaping_events(self, last_game_state, last_game_state, last_action)
    ...
    self.logger.info(
        ...
        f"Score: {self.round_score}  (avg {len(self.score_history)}: {avg_score:.2f})  "
        f"Reward: {self.episode_reward:.1f}  (avg: {avg_reward:.1f})  "
        f"coins={self.round_coins}  kills={self.round_kills}  "
        f"suicides={self.round_suicides}  survived={survived}"
    )
