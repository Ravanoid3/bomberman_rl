...
This includes decision making and the learning loop.
...
When the agent survives to the end of the round the last step arrives through *both* callbacks -- we detect that by step number and only
add the late ``SURVIVED_ROUND`` bonus rather than duplicating the transition.
"""
...
#: Per-step decision tracing is off by default: the framework enables DEBUG for
#: agent code unconditionally, and formatting a Q-vector plus flushing the log on
#: every step costs ~6% of training wall clock.  Set BOMBERMAN_VERBOSE=1 to get
#: the trace back when debugging a policy.
VERBOSE = os.environ.get('BOMBERMAN_VERBOSE', '') not in ('', '0', 'false')


def resolve_config(env_var, default):
    """Pick a configuration name from the environment, falling back to ``default``."""
...
def act(self, game_state):
    if game_state['round'] != self._seen_round:
        # end_of_round only fires while training, so reset per-round memory here
        self._seen_round = game_state['round']
        self.recent_positions.clear()

    phi, info = _features(self, game_state)
    ...
    # Explore vs greedy reinforcement learning
    if epsilon > 0 and self.rng.random() < epsilon:
        action_idx = _explore(self, info, allowed)
    else:
        action_idx = _greedy(self, _break_loops(self, info, q), allowed)
    ...
    # Printing actually takes quite some time. So only if verbose i set. Set BOMBERMAN_VERBOSE=1 to get the per-step trace back.
    if VERBOSE:
...
def setup_training(self):
    ...
    self._round_stats = _new_round_stats()
    self._td_errors = []
    ...
    if not self.train_log_path.exists():
        ...
        # A run that was interrupted (see the filesystem note in
        # experiments/README.md) resumes from its checkpoint, so the exploration
        # schedule has to resume too -- otherwise epsilon would jump back to
        # eps_start and undo the annealing already paid for.
        self.round_index = _rounds_already_logged(self.train_log_path)
        if self.round_index:
            self.logger.warning('resuming training log at round %d',
                                self.round_index)

    # The framework has no shutdown hook, and a run rarely stops exactly on a
    # save_every boundary -- atexit guarantees the last rounds are not lost.
...
def game_events_occurred(self, old_game_state, self_action, new_game_state, events):
    ...
    old_phi, old_info = _features(self, old_game_state)
    new_phi, new_info = _features(self, new_game_state)

    _learn_from_step(self, old_phi, old_info, self_action, new_phi, new_info,
                     events, old_game_state, new_game_state, terminal=False)
...
def end_of_round(self, last_game_state, last_action, events):
    if last_game_state is not None and last_action is not None:
        ...
        if already:
            # only SURVIVED_ROUND is new here; fold it into the pending window
            ...
            old_phi, old_info = _features(self, last_game_state)
            _learn_from_step(self, old_phi, old_info, last_action, None, None,
                             events, last_game_state, last_game_state, terminal=True)
...
def _save_quietly(self):
    """Best-effort checkpoint write, safe to run during interpreter shutdown."""
    try:
        self.model.save(self.checkpoint_path)
    except Exception:
        pass
...
# internals
# --------------------------------------------------------------------------
def _features(self, game_state):
    """Feature extraction with a one-step cache.

    ``act`` and ``game_events_occurred`` see the same states, so without the
    cache every state would be encoded twice -- a ~40% waste of training time.
    """
...
def _epsilon(self):
    cfg = self.cfg
    if not self.train:
        # Q_EVAL_EPS lets a sweep vary inference-time exploration without
        # editing configs; see the livelock discussion in experiments/README.md.
...
def cfg_allows(cfg, action_index):
    """Is this action part of the configured action space?"""
    return cfg['enable_bomb'] or action_index != gs.ACTION_INDEX['BOMB']


def _safe_actions(self, info, allowed):
    """
...
def _break_loops(self, info, q):
    ...
    not a reward hack. The penalty is expressed as a fraction of the current Q-spread so the same setting works
    whether values live around 1 or around 25.

    In order to avoid the agent just standing still half the time, ``WAIT`` is charged for the agent's own tile.
    """
...
def _allowed_actions(self, info):
    ...
    return _safe_actions(self, info, allowed) or allowed
...
def _explore(self, info, allowed):
    ...
    ``safe_explore``
        ...
        escape search *proves* survivable.  Without it, stage-2 rounds last ~10
        steps and nothing is ever learned (use arm ``task2_unsafe_explore``). It is
        left below 1 on purpose as the remaining fraction still lets the agent experience death >:3
    ``eps_greedy_bias``
    ...
    if p_safe > 0 and self.rng.random() < p_safe:
        safe = _safe_actions(self, info, allowed)
        if safe:
...
def _learn_from_step(self, old_phi, old_info, action, new_phi, new_info, events, old_state, new_state,
                     terminal):
    events = list(events)
...
def _is_in_lead(game_state):
    _, my_score, _, _ = game_state["self"]
    other_scores = [score for _, score, _, _ in game_state["others"]]
    return all(my_score > score for score in other_scores)
...
def _train_batches(self, n_batches):
    ...
    for _ in range(n_batches):
        ...
        # features stay int8 all the way through; each model casts as it needs
        # (the tabular model keys on the raw bytes, so the dtype must not drift)
...
def _augment(spec, phi, action, targets, rng):
    """Present each sampled transition under a random board symmetry.

    ``Q`` is equivariant under D4, so a transition observed once is evidence
    about all eight symmetric situations.  Drawing one group element per sample
    covers the whole orbit over the course of training and gives the same
    expected gradient as expanding all eight rows, at an eighth of the per-update
    cost -- which matters because this runs on every environment step.
    """
