...
* A bomb dropped at move ``k = 0`` is therefore lethal after moves
 ``BOMB_TIMER`` and ``BOMB_TIMER + 1``. Exactly four moves to leave a blast of
  radius three, so it can just be outrun.
...
try:  # the framework always puts the repository root on sys.path
...
# Sentinel for "no danger" / "unreachable".
NEVER = 99
...
def survivable_moves(field, lethal, start, blocked, kmax=KMAX):
    ...
    It retunrs a bitmask over the five non-bomb actions (bit ``a`` <-> ``ACTIONS[a]``, bit 4 = ``WAIT``).
    Bit ``a`` is set if some sequence of moves starting with ``a`` never puts the agent on a lethal tile within the horizon.

    Since the horizon covers every bomb on the board, a set bit ensures the survivability survivability under the
    ...
    if not lethal.any():
        ...
        # This is the common case (no bombs on the field) and thus needs to exit the function as quickly as possible
        # simply because it saves us a lot of time in training and evaluating. Was worth ~10% of training wall clock.
        mask = 1 << 4
...
def bfs(passable, start):
    ...
    every training step (millions of calls), and at 289 tiles the per-element numpy overhead actually dominated I think
    The list version measured ~2.5x faster.
    """
