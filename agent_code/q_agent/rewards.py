...
The reward handed to the learner is

    r = sum(table[event] for event in events)       # game + auxiliary events
      + step_penalty                                # prefer short solutions
      + gamma * Phi(s') - Phi(s)                    # potential-based shaping

The auxiliary events can bias the policy. Frankly, I think that a lot of these make sense to track though.
The *_noaux arms however, do not contain these.

The last term is potential-based: Allegedly, Ng et al. (1999) show that adding
``gamma * Phi(s') - Phi(s)`` for any state function ``Phi`` leaves the optimal
policy unchanged. So the uidance towards nearby objectives cannot, in the
limit, teach the agent something the true reward would not. I frankly am not sure about this one too much but that's
where I asked AI and got this recommended XD

Event name strings are the values defined in the framework's ``events.py`` where each constant there equals its own name
``experiments/selftest.py`` asserts that this stays true.
"""
...
def potential(cfg, info):
    """``Phi(s)``: minus the (capped) walking distance to the current objective.

    Objective priority mirrors what actually scores: visible coins first, then
    crates to open up new coins, then opponents.  Terminal states get ``0``.
    """
