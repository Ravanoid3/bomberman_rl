...
Design decisions:
* Everything is local and relative. No absolute coordinates appear, so the
  same encoding works on any board size and generalises across the arena.

* Everything is a small integer. The encoding is binary, which
  means the very same vector serves two models: the linear approximator reads
  it as a float vector, and the tabular model uses its raw bytes as a state key.
  Cuz of that Tabular vs. linear then differs *only* in the model class.
* Direction-dependent quantities live in contiguous four-slot blocks ordered ``UP, RIGHT, DOWN, LEFT`` so board
  symmetries act as index permutations (see symmetry.py).
* Which blocks are active is configurable. That's frankly just because early curriculum stages use a
  handful of blocks and I left a tiny state space at first, so that tabular learning converges in minutes.
  Later stages switch on the full set. It's kinda just kept around still XD
"""
...
ALL_BLOCKS = (
    # --- direction blocks ------------------------------------------------
    ('dir_free', 4, True),      # moving there is legal right now
    ('dir_lethal', 4, True),    # moving there dies to an explosion this step
    ('dir_safe', 4, True),      # moving there keeps a proven escape route
    ('dir_coin', 4, True),      # first step of a shortest path to a coin
...
# The crate layout only changes when a bomb goes off, but the gain map is
# needed on every step -- and it was ~0.3 ms per call.  Keyed by field content,
# so it is also correct to share between agents standing on the same board.
_GAIN_CACHE = {}
